...
export const authService = {
  async getUserById(id: string) { const user = await authRepository.findUserById(id); if (!user) throw new AppError(401, 'Invalid authentication session'); return publicUser(user); },

...
function CheckoutContent() {
  ...
  async function submitOrder() {
    ...
    try {
      ...
      try {
        ...
        const message = 'Your order was created, but we could not clear the cart.';
        setCartClearError(`${message} You can clear it from the cart page.`);
  ...
  if (order) return <div className="mx-auto max-w-6xl px-6 py-section lg:px-8"><Card className="mx-auto max-w-2xl text-center"><Badge tone="success">Order received</Badge><h1 className="mt-6 font-heading text-h1">Thank you for your order.</h1><p className="mt-5 text-body text-text-secondary">Your order has been created and is waiting for payment.</p><div className="mt-8 border-y border-border py-5 text-left text-small"><div className="flex justify-between"><span className="text-text-secondary">Order number</span><span className="font-mono text-primary">{order.id}</span></div><div className="mt-3 flex justify-between"><span className="text-text-secondary">Status</span><span className="font-mono text-accent">{formatEnumLabel(order.status)}</span></div><div className="mt-3 flex justify-between"><span className="text-text-secondary">Total</span><span className="font-mono text-primary">${Number(order.totalAmount).toFixed(2)}</span></div></div>{cartClearError && <p className="mt-5 text-left text-small text-error">{cartClearError}</p>}<Link href="/catalog"><Button className="mt-8">Continue shopping</Button></Link></Card></div>;
